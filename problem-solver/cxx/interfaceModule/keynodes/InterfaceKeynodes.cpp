#include "InterfaceKeynodes.hpp"

#include "sc-memory/sc_memory.hpp"

namespace interfaceModule
{
ScAddr InterfaceKeynodes::action_change_interface;
ScAddr InterfaceKeynodes::action_create_message_class_and_phrase_template;
ScAddr InterfaceKeynodes::action_create_class_instance;
ScAddr InterfaceKeynodes::action_create_class;
ScAddr InterfaceKeynodes::action_create_relation;
ScAddr InterfaceKeynodes::concept_header;
ScAddr InterfaceKeynodes::concept_main_part;
ScAddr InterfaceKeynodes::concept_footer;
ScAddr InterfaceKeynodes::concept_phrase;
ScAddr InterfaceKeynodes::concept_intent_possible_class;
ScAddr InterfaceKeynodes::concept_message;
ScAddr InterfaceKeynodes::concept_answer_on_standard_message_rule_priority_1;
ScAddr InterfaceKeynodes::concept_atomic_message;
ScAddr InterfaceKeynodes::concept_wit_entity;
ScAddr InterfaceKeynodes::nrel_component_color;
ScAddr InterfaceKeynodes::nrel_inclusion;
ScAddr InterfaceKeynodes::nrel_main_idtf;
ScAddr InterfaceKeynodes::nrel_answer;
ScAddr InterfaceKeynodes::nrel_wit_ai_idtf;
ScAddr InterfaceKeynodes::nrel_reply;
ScAddr InterfaceKeynodes::nrel_implication;
ScAddr InterfaceKeynodes::nrel_answer_pattern;
ScAddr InterfaceKeynodes::nrel_authors;
ScAddr InterfaceKeynodes::nrel_note;
ScAddr InterfaceKeynodes::nrel_subdividing;
ScAddr InterfaceKeynodes::nrel_definition;
ScAddr InterfaceKeynodes::nrel_definitional_domain;
ScAddr InterfaceKeynodes::nrel_combination;
ScAddr InterfaceKeynodes::nrel_first_domain;
ScAddr InterfaceKeynodes::nrel_second_domain;
ScAddr InterfaceKeynodes::rrel_entity;
ScAddr InterfaceKeynodes::rrel_color;
ScAddr InterfaceKeynodes::rrel_main_key_sc_element;
ScAddr InterfaceKeynodes::lang_ru;
ScAddr InterfaceKeynodes::lang_en;
ScAddr InterfaceKeynodes::atomic_logical_formula;
ScAddr InterfaceKeynodes::_message;
ScAddr InterfaceKeynodes::_reply_message;
}  // namespace interfaceModule

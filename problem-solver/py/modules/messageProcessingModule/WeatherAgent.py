...
class WeatherAgent(ScAgentClassic):
    ...
    def get_weather(self, entity_idtf: ScAddr, city_addr: ScAddr, country_addr: ScAddr) -> float:
        ...
        temperat = f'<div class="weather_wrapper layer" data-speed="1"><p style="text-align: center; margin-top: 10px;">{name}</p>{img}<p class="temperature" style="text-align: center; position: relative; top: -15px; font-size: 30px;">{temp}°C</p></div>'
